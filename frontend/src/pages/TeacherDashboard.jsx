                ...
                <div style={{ background: 'white', borderRadius: 28, padding: 24, marginBottom: 32, boxShadow: '0 2px 12px rgba(0,0,0,0.03)', border: '1px solid #f0f0f0' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
                    {data.ai_suggestions?.map((s, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 16, background: '#F5F3FF', borderRadius: 20, border: '1px solid #EDE9FE' }}>
            ...
            {tab === 'analytics' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: 24 }}>
                  ...
                  <div style={{ background: 'white', borderRadius: 28, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.03)', border: '1px solid #f0f0f0' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
                      <FiBarChart2 size={20} color="#8B5CF6" />
                      <h3 style={{ fontWeight: 700, color: '#1F2937' }}>Topic Difficulty Heatmap</h3>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                      {data.topic_heatmap?.map((t, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <span style={{ width: 110, fontSize: 14, fontWeight: 500, color: '#4B5563' }}>{t.topic}</span>
                          <div style={{ flex: 1, height: 8, background: '#F3F4F6', borderRadius: 999, overflow: 'hidden' }}>
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${t.difficulty}%` }}
                              transition={{ delay: i * 0.1, duration: 0.6 }}
                              style={{
                                height: '100%',
                                borderRadius: 999,
                                background: t.difficulty > 70 ? '#EF4444' : t.difficulty > 50 ? '#F59E0B' : '#10B981',
                              }}
                            />
                          </div>
                          <span style={{ width: 40, fontSize: 14, fontWeight: 700, color: '#4B5563', textAlign: 'right' }}>{t.difficulty}%</span>
                          <span
                            style={{
                              padding: '2px 8px',
                              borderRadius: 999,
                              fontSize: 11,
                              fontWeight: 600,
                              background: t.difficulty > 70 ? '#FEE2E2' : t.difficulty > 50 ? '#FEF3C7' : '#D1FAE5',
